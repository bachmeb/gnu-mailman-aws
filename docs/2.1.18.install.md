...
*Rather than set the hostname of this machine to match a resolvable web address, this tutorial assumes we want the machine to have a unique hostname that is different than any of the hostnames we define in the public DNS record. The mailman webserver will be addressble by a hostname defined in DNS, but the hostname of the AWS machine does not have to match the web address of the mailman web page. We should be able to configure mailman to run on a machine with any hostname, and still offer the mailman web page at a specific web address defined in DNS.*
...
*[Optional lookup tables that alias specific mail addresses or domains to other local or remote address. If you use this feature with indexed files, run "postmap /etc/postfix/virtual" after changing the file.](http://www.postfix.org/postconf.5.html#virtual_alias_maps)*
```
...
##### Use postmap to look for the alias in the virtual file
	postmap -q aka@spoonerspizza.com hash:/etc/postfix/virtual

##### Send a message from your local account to the alias address
	mail aka@[somedomain.whatever]

##### See if the message was delivered
	sudo cat /var/spool/mail/test

##### Send a message from your local account to the alias account
	mail aka
```
Subject: [time of day]
