...
##### Switch to your user account
	sudo su [your user account name]
...
##### Visit the test page for the Apache HTTP server from your SSH session 
	lynx localhost
...
sendmail 2520 root    4u  IPv4   9809      0t0  TCP 127.0.0.1:smtp (LISTEN)
```
...
##### Check to see if sendmail is set to run at boot time
	chkconfig --list
...
Connected to localhost.
Escape character is '^]'.
220 ur.localdomain ESMTP Postfix
```
##### Close the telnet connection
	ctrl-]
	q

##### Telnet to the EC2 IP address on port 25. The connection should be refused.
	telnet [ec2 ip address] 25
```
Trying [ec2 ip address]...
telnet: connect to address [ec2 ip address]: Connection refused
...
##### Check the default value for the following parameters
